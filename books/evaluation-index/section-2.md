...
## 正解率（Accuracy）再現率（Recall）適合率（Precision）F値（F-measure）
...
## 受信者操作特性（ROC）AUC（Area Under the Curve）
## シルエット係数 DaviesBouldin指数 CalinskiHarabasz指数
